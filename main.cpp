    ...
    for (;;)
    {
        ...
        if (nFrames % 10 == 0)
        {
            ...
            cout << "Frames captured: " << cv::format("%5lld", (long long int)nFrames)
                 ...
                 << "    Average time per frame: " << cv::format("%9.2f ms", static_cast<double>(t1 - t0) * 1000.0f / (N * getTickFrequency()))
                 << "    Average processing time: " << cv::format("%9.2f ms", static_cast<double>(processingTime) * 1000.0f / (N * getTickFrequency()))
                 << std::endl;
            t0 = t1;
            processingTime = 0;
        }
